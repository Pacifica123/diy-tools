...
GUI рисует картинку в памяти через SVG-превью. Большие схемы показываются в прокручиваемой области с масштабом `-`, `+`, `100%` и `По ширине`, чтобы они не сжимались до нечитаемого размера. Сохранение в PNG в первой версии не нужно. SVG можно сохранить вручную кнопкой `Сохранить SVG`.
...
- Простая раскладка: `src/mpl/core/layout.py`; там же защита от раздувания canvas на циклах.
- SVG: `src/mpl/core/svg_renderer.py`.
...
Это не полный Mermaid. В первой версии не обещаны sequence/class/state/gantt/ER-диаграммы, точное совпадение с Mermaid CLI, темы Mermaid, CSS-классы, интерактивные click-события и идеальная раскладка сложных циклических графов.
